...
def _field_stats(values: List[str]) -> Dict[str, Any]:
    ...
    if _is_numeric(non_empty):
        ...
        if n:
            mean = sum(nums) / n
            # population std dev (consistent with descriptive profiling convention)
...
@dataclass
class FieldInfo:
    """Metadata and descriptive statistics for a single dataset column."""
...
@dataclass
class DataCard:
    """A fully-populated dataset documentation card."""
...
class DatacardGenerator:
    """Generate a :class:`DataCard` from CSV files or Python data structures."""
    ...
    def generate_from_csv(self, path: Path) -> DataCard:
        """Load a CSV file and return a populated :class:`DataCard`."""
    ...
    def generate_from_dict(self, data: List[Dict[str, Any]]) -> DataCard:
        """Accept a list of row dicts and return a populated :class:`DataCard`."""
        return self._build_card([{k: str(v) for k, v in row.items()} for row in data])

    def generate(self, source) -> DataCard:
        """Polymorphic entry point — accepts a :class:`~pathlib.Path`, list of dicts, or column dict."""
...
def main(argv=None) -> int:
    ...
    if args.gui:
        from datacard_gui import launch_gui
        launch_gui()
        return 0
...
# alias required by pyproject.toml [project.scripts] entry
_cli = main
